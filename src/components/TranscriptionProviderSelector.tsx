...
import { TranscriptionProvider, TranscriptionModel, OutputFormat } from '../types/transcription';
...
const OUTPUT_FORMATS: { value: OutputFormat; label: string }[] = [
  { value: 'text', label: 'Texto (.txt)' },
  { value: 'srt', label: 'SubRip (.srt)' },
  { value: 'vtt', label: 'WebVTT (.vtt)' },
  { value: 'verbose_json', label: 'JSON (completo)' },
  { value: 'json', label: 'JSON (simples)' },
];
    ...
    <div className="mb-6 space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center">
              <FileText className="w-3 h-3 text-gray-500 mr-1.5" />
              <span className="text-gray-900 text-xs font-normal">
                {OUTPUT_FORMATS.find(f => f.value === selectedFormat)?.label || 'Texto (.txt)'}
              </span>
            </div>
            <ChevronDown className="w-3 h-3 text-gray-500" />
          </button>
          <div className="absolute z-10 w-full max-w-[220px] mt-1 bg-white border border-gray-300 rounded-md shadow-lg text-xs">
            {OUTPUT_FORMATS.map((format) => (
              <button
                key={format.value}
                type="button"
                className={`w-full px-2 py-1.5 text-left hover:bg-gray-50 transition-colors ${
